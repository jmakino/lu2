...
void print_parms(FILE* stream, PPARMS parms)
{
    ...
    fprintf(stream,"usetwocards=%d ncards=%d cardid=%d maxpt=%d vcommscheme=%d stress=%d\n",
	    ...
	    parms->vcommscheme,
	    parms->stress_factor);
    fprintf(stream,"bfsize=%d bflimit=%d bfscale=%d\n",
...
void set_parms(int argc, char * argv[], PPARMS parms)
{
    ...
    if (parms->bfscale<0)    libtestg_parms.bfscale = -parms->bfscale;
    fprintf(stderr,"Proc=%d matparms= %d %d %e\n", MP_myprocid(),
	    libtestg_parms.bfsize,
	    libtestg_parms.bflimit,
	    libtestg_parms.bfscale);
